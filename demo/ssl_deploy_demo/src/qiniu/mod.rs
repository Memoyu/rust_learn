...
pub struct QiniuClient<'a> {
    conf: &'a config::AccessConfig,
}

#[derive(Serialize, Deserialize, Debug)]
pub struct QiniuUploadCert {
    name: String,
    common_name: String,
    pri: String,
    ca: String,
...
#[derive(Serialize, Deserialize, Debug)]
pub struct QiniuCertificateDetailResponse {
    pub code: i32,
    pub error: String,
    pub cert: QiniuCertificateDetail,
}

#[derive(Serialize, Deserialize, Debug)]
pub struct QiniuCertificateDetail {
    name: String,
    common_name: String,
    dnsnames: Vec<String>,
    not_before: i32,
...
impl<'a> QiniuClient<'a> {
    ...
    // 获取证书详情
    pub async fn get_cert_detail(&self, cred_id: &str) -> Result<QiniuCertificateDetail> {
        ...
        let detail = client
            ...
            .send()
            .await?
            .json::<QiniuCertificateDetailResponse>()
            .await?;
    ...
    // 上传证书
    pub async fn upload_cert(&self) -> Result<QiniuCertificateDetail> {
        let url = "http://api.qiniu.com/sslcert";
        let req = QiniuUploadCert {
            name: "api_upload_cert".to_string(),
        ...
        let authorization = self.get_authorization(&url, None, Some(json))?;
